...
dbutils.widgets.text(
    "checkpoint_location",
    "/Volumes/fins_genai/unstructured_documents/checkpoints/ai_parse_document_workflow/02_3_extract_document_pages",
    "Checkpoint location",
)
dbutils.widgets.text("source_table_name", "parsed_documents_raw", "Source table name")
dbutils.widgets.text("table_name", "parsed_documents_pages", "Output table name")
...
print("📄 Starting Document Pages Extraction...")
...
# Extract page info from parsed documents
...
pages_df = (
    ...
    .select(
        col("path"),
        expr("posexplode(try_cast(parsed:document:pages AS ARRAY<VARIANT>))").alias(
            "id", "image_uri"
        ),
    )
    .select(
        col("path"),
        expr("id").alias("page_id"),
        expr("cast(image_uri:image_uri as string)").alias("image_uri"),
    )
)
...
query = (
    pages_df.writeStream.format("delta")
    .outputMode("append")
...
print("✅ Document pages extraction completed successfully!")
